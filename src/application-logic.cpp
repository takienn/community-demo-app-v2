...
ApplicationLogic::RequestReceiveSubscription(int nodeId, int timestep) {
   ...
   // This demo-app only sends one type of message (to stop a vehicles): the code is 0x01 ; if we need to send another type, please change to a different number
    ...
    mySubsStorage.writeUnsignedByte(EXT_HEADER_TYPE_UNICAST);
    ...
    //RSU ID is 5000
    mySubsStorage.writeInt(5000); // TODO remove the hard coded RSU ID as source of the UNICAST reception	
...
ApplicationLogic::ProcessReceptionOfApplicationMessage(vector<AppMessage>& messages){
    ...
    for (vector<AppMessage>::iterator it = messages.begin(); it != messages.end() ; ++it) {
        ...
        for (vector<AppMessage>::iterator it_ = m_messages.begin(); it_ != m_messages.end() ; ++it_) {
            ...
            if ((inputId == registeredId) && ((*it_).status == kScheduled)) {
                (*it_).status = kToBeApplied;  // JHNOTE: changed from kScheduled to kToBeApplied -> registrering for a traf_sim subs
                stringstream log;
                log << "APP --> [ApplicationLogic] ProcessReceptionOfApplicationMessage() Message " << (*it).messageId << " is registered and changed status from kScheduled to kToBeApplied";
                Log::Write((log.str()).c_str(), kLogLevelInfo); 
            } 
            else {
                stringstream log;
                log << "APP --> [ApplicationLogic]  ProcessReceptionOfApplicationMessage() Message " << (*it).messageId << " not registered.";
...
ApplicationLogic::CheckForRequiredSubscriptions (int nodeId, int timestep){
  ...
  for (vector<AppMessage>::iterator it_ = m_messages.begin(); it_ != m_messages.end() ; ++it_) {
       if (((*it_).status == kToBeScheduled) && ((*it_).senderId == nodeId)) { // we need to send an APP_MSG_SEND and it should be a subscription created by me
           ...
           //Command length
	   mySubsStorage.writeUnsignedByte(1 + 1 + 1 + 1 + 1 + 1 + 1 + 4 + 1 + 2 + 4 + 1 + 1 + 1 + 4);
	   //Command type
	   mySubsStorage.writeUnsignedByte(CMD_ASK_FOR_SUBSCRIPTION);
	   ...
	   mySubsStorage.writeUnsignedByte(2);     // unsigned int msgLifetime = 2;
           mySubsStorage.writeShort(500); // m_app_Msg_length
	   mySubsStorage.writeInt((*it_).messageId); //sequence number
	   mySubsStorage.writeUnsignedByte(EXT_HEADER_TYPE_UNICAST); // CommMode
	   mySubsStorage.writeUnsignedByte(EXT_HEADER__VALUE_BLOCK_IDs); // CommMode
	   ...
	   mySubsStorage.writeInt((*it_).destinationId); //Destination node to set its maximum speed.
	   mySubsStorage.writeFloat(0.0); //Set Maximum speed (0.0 in this case)
           break;
     }
     else if (((*it_).status == kApplied) && ((*it_).senderId == 5000)) { // we need to send an APP_CMD_TRAFFIC_SIM subscription
...
ApplicationLogic::SendBackExecutionResults(int senderId, int timestep)
{
   ...
   if (timestep >= m_appStartTimeStep && senderId == 5000) {  // restrict the command to the RSU only (ID RSU is 5000)
